...
6. **C++ accelerator: remaining work after PR #37.** Phases 1, 2, 3, and 4 landed: cross-edge/cross-wire quadrature (PR #11), same-edge regularized quadrature (PR #12), Z matrix assembly fast path (PR #13), and Z matrix assembly general path for junction geometries (PR #37). Both triangular solver paths' Python overhead is now down to glue (geometry setup, the call sequence, np.linalg.solve, J build). Remaining wins, ranked by current Amdahl share:
   ...
   - **6d. Batched LU solve.** Post-PR-#37, on the 5-band fan dipole `np.linalg.solve` is ~27% of the sweep; on inverted V N=80 it's ~18%. **J build (item 6b) is the dominant cost on all geometries**, not the solve. Per-thread LU-factorize-then-solve via LAPACK's `zgetrf`/`zgetrs` directly (parallel over k) might still beat numpy's stacked call for the (n_k=41, M~160-425) shape — OpenMP over the n_k axis gives near-linear scaling. Estimated 2-4× on the solve, hence ~1.2-1.5× end-to-end. Lower priority than 6b now.

   - **6f. `np.zeros_like` on the J tensors.** Post-PR-#37 the fused J-build no longer pre-allocates J tensors in Python (the C++ kernel allocates them as its output). Item likely retired — re-investigate only if needed.

   - **6b. Vectorized `cexp(-jkR)` via SLEEF or Intel SVML.** *Now the most impactful remaining lever.* The C++ quadrature kernels already call libmvec's AVX2 vectorized `cos_avx2`/`sin_avx2` (4 doubles per inst). Switching to AVX-512 sincos via SLEEF or Intel SVML (8 doubles per inst) is realistically another 1.5-2× on the sincos portion of each quadrature kernel. VTune on fan-dipole /sweep post-PR-#37 shows `seg_seg_quad_batch_3d._omp_fn.0` at 15.5 thread-sec + `cos_avx2` 9.1 + `sin_avx2` 6.5 = ~31 thread-sec of compute; the sincos is ~50% of the kernel and the kernel is ~44% of /sweep. So 2× on sincos = ~22% on J build = ~10% on /sweep e2e. Adds a build dependency (libsleef or icx).
