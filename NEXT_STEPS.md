...
6. **C++ accelerator: remaining work after PR #13.** Phases 1, 2, and 3 landed: cross-edge/cross-wire quadrature (PR #11), same-edge regularized quadrature (PR #12), and Z matrix assembly (PR #13 — *but only the non-junction fast path*; see 6g). The non-junction triangular solvers' Python overhead is now down to glue (geometry setup, the call sequence, np.linalg.solve). Remaining wins, ranked by current Amdahl share:

   - **6g. assemble_Z for junction geometries.** *Now the dominant cost on /sweep for fan dipole.* PR #13's `assemble_Z` only handles the non-junction fast path (`_assemble_Z_single` / `_assemble_Z_batch`, indexed by per-basis `left_seg, right_seg`). Junction geometries fall through to `_assemble_Z_general_single` / `_assemble_Z_general_batch`, which run in pure Python using the more general `(support_seg, support_L, support_R)` per-basis-per-wing arrays. cProfile of a 41-freq 5-band fan-dipole sweep (n_per_wire=21): `_assemble_Z_general_batch` is **5114 ms tottime out of 6131 ms total — 83% of the sweep**. The other pysim hot paths (Z assembly is dominant; build_J 7%; solve 5%) are dwarfed by this.

     The general path's math is the same scalar/vector-potential blend as PR #13's, just with up to two wings per basis instead of one fixed (left, right) stencil and per-wing `(L_ma, slope_ma)` carrying the basis level and slope. Concretely, for each `(k, m, n)` triple, the inner loop is:
     ```
     S    = Σ_a Σ_b  slope[m,a]*slope[n,b]  * J00[k, support_seg[m,a], support_seg[n,b]]
     I_A  = Σ_a Σ_b  td_blk[m,n,a,b] * (  L[m,a]*L[n,b]  * J00
                                        + L[m,a]*slope[n,b]*J01
                                        + slope[m,a]*L[n,b]*J10
                                        + slope[m,a]*slope[n,b]*J11 )
     Z[k,m,n] = jωμ·I_A + S/(jωε)
     ```
     where the (a, b) inner loop is 2×2 and the (m, n, k) outer loops are the parallel axes. Same OpenMP `parallel for collapse(2)` over `(k, m)` as PR #13.

     **Expected speedup**: extrapolating from PR #13's general experience (Python assembly ~33–66× → C++ assembly on equivalent-size matrices), the general path should drop from ~5 s → ~100 ms on this geometry. End-to-end sweep at 41 freqs should go from ~4.3 s (adaptive-chunked) → ~1 s, hence ~4× speedup on /sweep for fan dipole. *This is the single biggest remaining win on multi-band geometries.*

     **API target**: new `assemble_Z_general(J00, J10, J01, J11, h_per_seg, td_all, support_seg, support_L, support_R, omega_array, eps, mu) -> Z` in `_accelerators.cpp`. The existing `assemble_Z` can stay as the fast path for interior-bases-only geometries, or both paths could be unified by passing the general form and letting the kernel handle the degenerate (one-wing, contiguous-stencil) case efficiently — TBD when implementing. Validation: bit-exact with `_assemble_Z_general_batch` on the 5-band fan dipole and on the inverted V (where it should agree with the existing `assemble_Z` to roundoff). Existing 28 Python tests gate.

   - **6d. Batched LU solve.** *Was the dominant cost on /sweep for inverted V before 6g moved the needle on fan dipole.* `compute_impedance_swept` calls `np.linalg.solve(Z, v)` on a `(n_k, M, M)` matrix — already batched, **288 ms at N=80 (50%)** and **1239 ms at N=160 (60%)**. Per-thread LU-factorize-then-solve via LAPACK's `zgetrf`/`zgetrs` directly (parallel over k) should beat numpy's stacked call for our (n_k=41, M~160-320) shape: LAPACK's batched path may serialize internally, and OpenMP over the n_k axis gives near-linear scaling on this shape. Estimated 2–4× on the solve, hence ~1.5–2× end-to-end at N=160. Next-priority after 6g.
